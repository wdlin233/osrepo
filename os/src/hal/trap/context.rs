...
pub struct TrapContext {
    ...
    #[cfg(target_arch = "riscv64")] pub sstatus: Sstatus, // Supervisor Status Register
    #[cfg(target_arch = "loongarch64")] pub prmd: usize, //控制状态寄存器
    pub sepc: usize, // Supervisor Exception Program Counter，异常处理返回地址
    #[cfg(target_arch = "riscv64")] pub kernel_satp: usize, // Token of kernel address space
    #[cfg(target_arch = "riscv64")] pub kernel_sp: usize, // Kernel stack pointer of the current application
...
impl TrapContext {
    ...
    pub fn app_init_context(
        ...
        #[cfg(target_arch = "riscv64")] let mut sstatus = sstatus::read();
        // set CPU privilege to User after trapping back
        #[cfg(target_arch = "riscv64")] sstatus.set_spp(SPP::User);
        
        // 设置为用户模式,trap使用ertn进入用户态时会被加载到crmd寄存器中
        #[cfg(target_arch = "loongarch64")] prmd::set_pplv(CpuMode::Ring3);

        let mut cx = Self {
            x: [0; 32],
            #[cfg(target_arch = "riscv64")] sstatus,
            #[cfg(target_arch = "loongarch64")] prmd: prmd::read().raw(),
            sepc: entry,  // entry point of app
            #[cfg(target_arch = "riscv64")] kernel_satp,  // addr of page table
            #[cfg(target_arch = "riscv64")] kernel_sp,    // kernel stack
