...
use crate::{
    error::Error,
    fs::{File, Inode, Kstat, OpenFlags, Socket, StMode},
...
impl File for SocketFile {
    ...
    fn fstat(&self) -> Kstat {
        let now_sec = (get_time_ms() / 1000) as isize;
        let now_nsec = ((get_time_ms() % 1000) * 1_000_000) as usize;

        Kstat {
            st_dev: 0,                      // sockfs 无设备号
            st_ino: self.ino(),             // 匿名 inode 号
            st_mode: StMode::FSOCK | 0o666, // S_IFSOCK | rw-rw-rw-
            st_nlink: 1,
            st_uid: 0,
            st_gid: 0,
            st_rdev: 0, // 不是字符/块设备
            __pad: 0,
            st_size: 0,       // socket 无尺寸概念
            st_blksize: 4096, // 常见页大小
            __pad2: 0,
            st_blocks: 0, // 不占磁盘块
            st_atime: now_sec,
