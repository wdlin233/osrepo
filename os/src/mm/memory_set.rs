...
use crate::config::{DL_INTERP_OFFSET, MEMORY_END, MMAP_TOP, MMIO, PAGE_SIZE, TRAMPOLINE};
use crate::data_flow;
...
impl MemorySetInner {
    ...
    pub fn from_elf(elf_data: &[u8]) -> (Self, usize, usize, Vec<Aux>) {
        ...
        info!("(from_elf): entry point: {:#x}", entry_point);
        let (max_end_vpn, header_va) = memory_set.map_elf(&elf, VirtAddr(0));
        ...
        info!("(from_elf): max end va: {:#x}", user_heap_bottom);
        // guard page，用户栈
        ...
        info!("(from_elf): user heap bottom: {:#x}", user_heap_bottom);
    ...
    fn map_elf(&mut self, elf: &ElfFile, offset: VirtAddr) -> (VirtPageNum, VirtAddr) {
        ...
        for i in 0..ph_count {
            ...
            if ph.get_type().unwrap() == xmas_elf::program::Type::Load {
                ...
                info!(
                    "mapping program header {}: start_va = {:#x}, end_va = {:#x}",
                    i, start_va.0, end_va.0
                );
                #[cfg(target_arch = "riscv64")]
