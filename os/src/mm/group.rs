...
struct GroupInner {
    //该组内共享的帧
    pub shared_frames: BTreeMap<VirtPageNum, FrameTracker>,
    //该组内剩余的maparea数量
...
impl GroupManager {
    ...
    pub fn add_frame(&mut self, id: usize, vpn: VirtPageNum, frame: FrameTracker) {
        self.groups
            .get_mut(&id)
            .unwrap()
            .shared_frames
    ...
    //查找共享帧
    pub fn find(&mut self, id: usize, vpn: VirtPageNum) -> Option<FrameTracker> {
        if id == 0 {
            return None;
        }
        //get(&id)必定成功
