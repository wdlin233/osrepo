...
use alloc::sync::Arc;
use log::debug;
use polyhal_trap::{trap::TrapType, trapframe::TrapFrameArgs};
use crate::trap::MachineContextConversion;
pub use sigact::*;
pub use signal::*;

use crate::{
    config::USER_STACK_SIZE, trap::{MachineContext, UserContext}, mm::translated_refmut, task::{
...
pub fn handle_signal(signo: usize, trap_type: TrapType) {
    let process = current_task().unwrap();
    ...
    if sig_action.customed {
        setup_frame(signo, sig_action, trap_type);
    } else {
...
/// 并恢复原来内核栈的内容
pub fn setup_frame(signo: usize, sig_action: KSigAction, trap_type: TrapType) {
    error!("(setup_frame)");
    ...
    if !sig_action.act.sa_flags.contains(SigActionFlags::SA_SIGINFO) {
        ...
        //*translated_refmut(user_sp as *mut MachineContext) = trap_cx.as_mctx();
        
        // signal mask
        user_sp = user_sp - size_of::<SignalFlags>();
        *translated_refmut(user_sp as *mut SignalFlags) = inner.sig_mask;
        
        // 不是 sigInfo
        user_sp = user_sp - size_of::<usize>();
        *translated_refmut(user_sp as *mut usize) = 0;
    } else {
        ...
        let _sig_size = sig_sp - (inner.user_stack_top - USER_STACK_SIZE);
        // debug!("sig_size={:#x}", sig_size);
        // *translated_refmut(uctx_addr as *mut UserContext) = UserContext {
        //     flags: 0,
        //     link: 0,
        ...
        *translated_refmut(siginfo_addr as *mut SigInfo) = SigInfo::new(signo, 0, 0);
        // a1
        ...
        // 是 sigInfo
        user_sp = user_sp - size_of::<usize>();
        *translated_refmut(user_sp as *mut usize) = usize::MAX; 
    }

    // checkout(Magic Num)
    user_sp -= size_of::<usize>();
    *translated_refmut(user_sp as *mut usize) = 0xdeadbeef;
    // a0
...
pub fn restore_frame() -> SyscallRet {
    ...
    let checkout = *translated_refmut(user_sp as *mut usize);
    assert!(checkout == 0xdeadbeef, "restore frame checkout error!");
    user_sp += size_of::<usize>();

    // sigInfo标志位
    let sa_siginfo = *translated_refmut(user_sp as *mut usize) == usize::MAX;
    user_sp += size_of::<usize>();

    if !sa_siginfo {
        // signal mask
        // inner.sig_mask = get_data(token, user_sp as *const SignalFlags);
        inner.sig_mask = *translated_refmut(user_sp as *mut SignalFlags);
        user_sp += size_of::<SignalFlags>();
        // Trap cx
        let _mctx = *translated_refmut(user_sp as *mut MachineContext);
